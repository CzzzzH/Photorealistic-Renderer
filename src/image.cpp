...
double ReversedClampColorComponent(unsigned char c)
{
    double tmp = c / 255.0;
...
Image* Image::LoadBMP(const char *filename)
{
    ...
    for (int i = 0; i < height; i++)
    {
        ...
        {
            int ipos = (width * i + j);
            data[ipos] = Vector3f(ReversedClampColorComponent(line[3 * j + 2]),
                                  ReversedClampColorComponent(line[3 * j + 1]),
                                  ReversedClampColorComponent(line[3 * j]));
        }
    }
